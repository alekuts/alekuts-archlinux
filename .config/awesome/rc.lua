...
beautiful.init("/home/alekuts/.config/awesome/theme.lua")
...
awful.spawn.with_shell("setxkbmap -layout us,ua")
...
local function toggle_layout()
    awful.spawn.easy_async_with_shell([[
        ...
        -- Тут можна додати сповіщення, щоб бачити, що спрацювало
        naughty.notify({ text = "layout changed", timeout = 2 })
    end)
end

screen.connect_signal("request::desktop_decoration", function(s)
    -- Each screen has its own tag table.
    awful.tag({ "main", "second", "background", "void" }, s, awful.layout.layouts[1])
...
awful.keyboard.append_global_keybindings({
    awful.key({ modkey, "Mod1" }, "Tab", function() awful.spawn("rofi -show run") end,
...
naughty.connect_signal("request::display", function(n)
    naughty.layout.box {
        notification = n,
        -- Створюємо шаблон для віджетів всередині сповіщення
        widget_template = {
            {
                {
                    {
                    ...
                    margins = 10, -- Відступи від країв вікна
            ...
            strategy = "max",
            width    = 300, -- Можеш підібрати ширину під себе
